...
| `--indexer-password` | `admin` | Password of that account. |

`--indexer-password` defaults to the password the indexer image ships, so a deployment that has not been through the credential rotation needs no options at all. **After rotating the `admin` password, the suite fails until you pass the new one:**

```bash
pytest tests/k8s_pytest.py -v --deployment-type local \
  --indexer-password '<the new admin password>'
```
